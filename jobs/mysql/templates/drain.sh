...
if ! ps -p "$(</var/vcap/sys/run/mysql/mysql.pid)" >/dev/null; then
  echo "mysql is not running: OK to drain" >> "${LOG_DIR}/drain.log"
...
for NODE in "${CLUSTER_NODES[@]}"; do
  status_code=$(curl -s -o "${LOG_DIR}/drain.log" -w "%{http_code}" "$NODE:$GALERA_HEALTHCHECK_PORT")
  if [[ $status_code -eq 000 || $status_code -eq 200 ]]; then
    continue
  else
    echo "galera heathcheck returned $status_code; drain failed" >> "${LOG_DIR}/drain.err.log"
    exit -1
  fi
done
